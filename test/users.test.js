		...
		describe('POST', function() {
			...
			it('Should reject users with non-string username', function() {
				...
				return chai
					...
					.then(res => {
						expect(res).to.have.status(422);
						expect(res.body.message).to.equal('Invalid \'username\' entered');
					});
			});
			it('Should reject users with non-string password', function() {
				const testUser = {
					username: 'hughwilliams',
					fullName: 'Sam Smith',
					password: 12
				};
				return chai
					...
					.then(res => {
						expect(res).to.have.status(422);
						expect(res.body.message).to.equal('Invalid \'password\' entered');
					});
			});
